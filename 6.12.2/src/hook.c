...
int fake_RSA_public_decrypt(int flen, unsigned char *from, unsigned char *to, RSA *rsa, int padding);
...
__attribute__((constructor)) void unraid_init() {
  rsa_hook = subhook_new(RSA_public_decrypt, fake_RSA_public_decrypt, 0);
  subhook_install(rsa_hook);
...
int fake_RSA_public_decrypt(int flen, unsigned char* from, unsigned char* to,
                       RSA* rsa, int padding) {
